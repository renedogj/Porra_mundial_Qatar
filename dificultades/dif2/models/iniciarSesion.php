...
if(checkBlackList($auxEmail, $blackList) && checkBlackList($auxPassword, $blackList)) {
	...
	if(count($usuario) != 0){
		...
		setcookie("id", $usuario["id"], time() + 86400, "/dificultades/dif2/");
		setcookie("nombre", $usuario["nombre"], time() + 86400, "/dificultades/dif2/");
		setcookie("puntuacion", $usuario["puntuacion"], time() + 86400, "/dificultades/dif2/");
		setcookie("email", $usuario["email"], time() + 86400, "/dificultades/dif2/");
	}else{
		$json["error"] = true;
	}	
}else{
